...
export const TopAlbums = ({ username, viewMore }: Props) => {
  ...
  const {
    ...
    queryKey: ['top-albums', username, timeframe, limit, page],
    queryFn: () => fetchLastFmUserTopAlbums(username, timeframe, limit, page)
  })

  if (isPending) {
    return (
      <StatsContainer page={page} setPage={setPage} type="albums" viewMore={!!viewMore} setMode={setMode} mode={mode} username={username}>
        {Array(limit)
          .fill(0)
          .map((_, index) => (
  ...
  return (
    <StatsContainer page={page} setPage={setPage} type="albums" viewMore={!!viewMore} setMode={setMode} mode={mode} username={username}>
      {mode === 'grid' && <DataTable columns={AlbumsColumns} data={albums} />}
      {mode === 'list' && (
        <div className="flex flex-wrap gap-x-2 gap-y-4 justify-evenly items-center">
          {albums && albums?.map((album, index) => <Album album={album} key={index} />)}
        </div>
      )}
    </StatsContainer>
  )
