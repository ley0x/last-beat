...
import {
  ...
  topsterGapAtom,
  topsterHeightAtom,
  topsterRoundCornersAtom,
  topsterShowTitlesAtom,
  topsterTitleAtom,
  topsterWidthAtom
} from '@lib/store'
...
export const TopsterOptions = () => {
  ...
  useEffect(() => {
    setAlbums(albums =>
      [...albums, ...Array(topsterWidth * topsterHeight).fill(null)].slice(0, topsterWidth * topsterHeight)
    )
  ...
  return (
    <>
      ...
      <div className="flex justify-between w-full flex-wrap gap-1">
        ...
        <Switch id="gaps" onCheckedChange={() => setTopsterGap(topsterGap => !topsterGap)} checked={topsterGaps} />
      </div>
      <div className="flex justify-between w-full flex-wrap gap-1">
