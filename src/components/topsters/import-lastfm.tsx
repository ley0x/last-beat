...
export const ImportLastfm = () => {
  ...
  useEffect(() => {
    if (albums) {
      setAlbums(
        albums.map(album => ({ artist: album.artist.name, url: album.url, name: album.name, image: album.image }))
      )
    }
  }, [albums, setAlbums])
  ...
  return (
    <Card className="gap-4 bg-card/20">
      <CardHeader>
        ...
        <CardDescription>Import your last.fm most played albums.</CardDescription>
      </CardHeader>
