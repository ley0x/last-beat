...
import { SortableCell } from '@topsters/sortable-cell'
  ...
  return (
    <div
      className={cn(
        ...
        GRID_CLASSES.rows[topsterHeight],
        topsterGap && 'gap-[1px] lg:gap-[2px] xl:gap-1'
      )}
      style={{
    ...
    >
      {albums.map((album, idx) => (
        <SortableCell key={getCellId(album, idx)} id={getCellId(album, idx)} album={album} />
      ))}
    </div>
  )
}
