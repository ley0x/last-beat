...
export const LyricsCanvasContainer = () => {
  ...
  const handleExport = useCallback(() => {
    ...
    toPng(ref.current, { cacheBust: true, pixelRatio: 2, skipFonts: true })
      .then(dataUrl => {
        const name = `${slugify(getTrackArtist(), { lower: true }) || 'artist'}-${
          slugify(getTrackName(), { lower: true }) || 'song'
        }-card.png`
      ...
      <li className={cn('min-h-[14rem] list-none')}>
        <div className="relative h-full rounded-sm border-[0.75px] border-border p-2 md:rounded-sm md:p-3">
          ...
          <div
            ...
            id="lyrics-canvas"
            className={cn(
              'mx-auto relative cursor-pointer shadow-lg rounded-sm overflow-hidden',
              shapeStyle[shape],
              bgColorMap[bgColor]
            )}
          >
